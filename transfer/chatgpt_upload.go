...
// bytes from a reader through the consumer's authenticated upload path, owning auth and
...
// operator-configured maxRelayBytes) so the OpenAI file branch honors the consumer's
...
// production path passes nil to use the SSRF-guarded client).
