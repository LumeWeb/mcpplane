...
// TransportKind is re-exported from model so both packages agree on the
// same type. The canonical definition lives in model to avoid import cycles.
type TransportKind = model.TransportKind

const (
	TransportStdio  TransportKind = model.TransportStdio
	TransportHTTP   TransportKind = model.TransportHTTP
	TransportOpenAI TransportKind = model.TransportOpenAI
)
...
// and the description builders — so a supported mode is never absent from the
// schema and an unsupported mode is never surfaced to a model.
func SourceModeEnumFromFeatures(fs model.FeatureSet) []any {
	var modes []any
	for _, m := range [...]struct {
		mode FileSourceMode
		feat model.Feature
	}{
		{SourcePath, model.FeatSourcePath},
		{SourceMint, model.FeatSourceMint},
		{SourceURL, model.FeatSourceURL},
		{SourceData, model.FeatSourceData},
	} {
		if fs.Has(m.feat) {
...
// set belongs to. It is the inverse of the transport's mechanism features:
// the mechanism source features (path/mint/url/data) pin the transport. It is
// used by schema transforms so the upload_file/vault_put_file source.mode enum
// and archive_mode prose stay bound to what the transport's handler actually
...
func TransportKindFromFeatures(fs model.FeatureSet) TransportKind {
	switch {
	case fs.Has(model.FeatSourcePath):
		return TransportStdio
	case fs.Has(model.FeatSourceMint):
		return TransportHTTP
	case fs.Has(model.FeatSourceURL) || fs.Has(model.FeatSourceData):
...
// generic platform profile (resolved by canimcp, the host-envelope library
// that owns transport→profile resolution) so capabilities and the schema share
// one feature-driven source of truth. Unknown transports advertise nothing
// (nil-for-unknown), since the generic profile degrades them to stdio-generic.
func SourceModeEnumValues(t TransportKind) []string {
	switch t {
	case TransportStdio, TransportHTTP, TransportOpenAI:
		var out []string
		for _, m := range SourceModeEnumFromFeatures(featuresForTransport(t)) {
...
// featuresForTransport resolves the feature set of the transport's generic
// profile. canimcp owns the transport→profile machinery (its generic profiles
// merge the transport-mechanism features — path/mint/url/data, sink
// reachability, co-location — by construction), so transfer never re-declares
// that mapping; the string feature vocabulary is identical to model's.
func featuresForTransport(t TransportKind) model.FeatureSet {
	profile := canimcp.ProfileForTransport(canimcp.TransportKind(t))
	features := make(model.FeatureSet, len(profile.Features))
	for f, on := range profile.Features {
		features[model.Feature(f)] = on
	}
	return features
}
