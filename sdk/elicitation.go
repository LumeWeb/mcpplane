...
//
// sdk puts a host-owned, SDK-neutral layer in front of that: handlers
...
func CallToolResultFromElicitation(spec model.ElicitationSpec) *mcp.CallToolResult {
	...
	if spec.URL != "" || spec.ElicitationID != "" {
		elicit.Mode = "url"
		elicit.URL = spec.URL
		// NOTE: URL-mode elicitation params no longer carry an
		// elicitationId; the client learns the outcome by retrying the
		// original request, so we do not emit it on the wire. The field is
		// kept on ElicitationSpec for callers that need to thread an id
		// through an out-of-band flow internally.
	} else {
