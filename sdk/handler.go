...
// This file owns the tool-handler adapter chain: it turns a host-owned
...
type HandlerDeps struct {
	...
	// LogStart optional hook called before invoking the host handler.
	LogStart func(name string, args map[string]any)
	// LogEnd optional hook called after invoking the host handler.
...
// ToolResult converts a host-owned tool result into an SDK result,
...
// AdaptToolHandler wraps a host-owned handler into the SDK tool handler the
// server registers. The arguments arrive on the wire as raw JSON; they are
// unmarshalled into a plain map for the host-owned handler. When the call is
...
// RegisterTool registers one host-owned tool on the server, adapting its
