...
// direct-registered tool handler (upload_file, upload_url, upload_data,
// put_file, download_file, get_file).
//
// The result is kept in StructuredContent exactly as the handler produced it
// (a typed download result stays a struct so the MCP App reads the top level),
// while Text carries the result's canonical JSON so a TEXT-ONLY agent sees the
// same data a structured-content consumer reads — the CID for uploads, the
...
// "status" member (e.g. a download result with status:"ok") is not clobbered —
// its own status is kept and the envelope is simply its own JSON. Scalars and
// arrays are wrapped under "value".
//
