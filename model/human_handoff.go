...
// builder (NeedsHumanResult, OOB hand-off builders, resume continuations)
// routes through it so the text always carries the URL, handle and resume tool
// the agent needs. The handle is always surfaced when present: a caller that
// only sees text must still be able to poll the matching resume tool.
...
// about os.Stdin; the agent-facing tools are agent-safe OOB hand-offs that
// never touch stdin.
