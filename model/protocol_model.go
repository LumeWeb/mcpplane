...
// ToolHandler executes a catalog tool without depending on an MCP SDK.
...
// token (`oauth2`), and which scopes the token must carry. Servers deployed as
// a protected resource default their tools to oauth2 with no application
// scopes; individual tools that may run anonymously declare noauth.
...
// ToolDescriptor describes a server-owned tool.
type ToolDescriptor struct {
	...
	// the adhoc name-allowlist predicate used to filter the curated
	// registration loop.
	DirectVisible bool
	...
	// applies the server default (oauth2, no scopes).
...
// ResourceDescriptor describes a known resource.
...
// ResourceTemplateDescriptor describes a resource URI template.
...
// ToolDescriptorForHandler attaches a handler to a descriptor.
func ToolDescriptorForHandler(entry *ToolEntry, handler ToolHandler) ToolDescriptor {
