...
// Characterization tests for CheckTreeSize / CheckDirectorySize, ported from
// the source repo's internal/mcp/vault/tree_size_test.go (characterization-first
// so the size-cap semantics are pinned in this repository before any further
// refactor).

// buildTreeFS writes a small directory tree and returns its path.
...
// TestCheckTreeSizeRejectsSingleOversize verifies a single file over the cap
...
// TestCheckTreeSizeRejectsAggregate verifies the aggregate of several
// under-cap files is rejected once their sum exceeds the cap.
...
// pre-flight FOLLOWS symlinks (os.Stat), so a symlink pointing at a file
// larger than maxBytes is rejected — whereas an fs.WalkDir over os.DirFS
// would lstat the entry and skip it, letting the oversized target bytes
// bypass the cap while still being transferred.
